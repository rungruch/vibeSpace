...
import { Form, Input, Switch, Card, Row, Col, Typography, Space, Divider, Button, notification, message, Select, Modal, Popconfirm } from 'antd';
...
import { PostSettings } from "../../../../interface.ts";
import { EditorMode } from "../../../../enum.ts";
import { createPost, fetchAllPostsCategory, createPostCategory, deletePostCategory, getPostById } from "../../../../../api/posts.ts";
...
export default function PostCreate() {
  ...
  const handleSave = async (settings: PostSettings, content: string) => {
    ...
    try {
      const createdPost = await createPost(pageData);
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      const newCategory = await createPostCategory({ name: newCategoryName.trim() });
  ...
  const handleDeleteCategory = async (categoryId: string, categoryName: string) => {
    try {
      ...
      // Update the categories list
      setPostCategories(prev => prev.filter((cat: any) => cat.id !== categoryId));
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
                ...
                >
                  <Select
                    ...
                    filterOption={(input, option) =>
                      postCategories.find((cat: any) => cat.id === option?.value)?.name?.toLowerCase().includes(input.toLowerCase())
                    }
                    showSearch
                    allowClear
        ...
        <div>
          <Input
            ...
            addonAfter={<Button onClick={() => setSelectFileModal(true)}>เลือกจากไฟล์</Button>}
          />
